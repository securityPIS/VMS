...
const QueueTab = ({ visits, onCheckIn, onReject, showLocation = false }) => (
  <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 animate-in fade-in slide-in-from-bottom-2">
    {visits.map((v) => (
      <div key={v.id} className="bg-white/85 backdrop-blur-xl p-6 rounded-[28px] shadow-card border border-line hover:shadow-md transition-shadow">
        <div className="flex justify-between items-start mb-4">
          <div>
            <h3 className="text-xl text-display">{v.name}</h3>
            <p className="text-sm text-ink-muted">{v.asal}</p>
        ...
        <div className="bg-ink/[0.03] p-4 rounded-[16px] mb-6">
    ...
    {visits.length === 0 && (
      <div className="col-span-full py-20 flex flex-col items-center justify-center text-[#74777F] bg-white/85 backdrop-blur-xl rounded-[28px] shadow-card border border-line border-dashed">
