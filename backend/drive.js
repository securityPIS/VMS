...
function findPhotoAccess(photoId) {
  ...
  for (let i = 0; i < visits.length; i++) {
    if (photoRefMatches(visits[i].selfie_url, photoId)) return { kind: 'visit', row: visits[i] };
  }

  const packages = readRows(SHEETS.PACKAGES);
  for (let j = 0; j < packages.length; j++) {
    if (photoRefMatches(packages[j].photo_url, photoId)) return { kind: 'package', row: packages[j] };
  }

  const visitors = readRows(SHEETS.VISITORS);
  for (let k = 0; k < visitors.length; k++) {
    if (photoRefMatches(visitors[k].ktp_photo_url, photoId)) return { kind: 'visitor', row: visitors[k] };
  }
  return null;
}
